#!/usr/bin/python3
"""
A python script based on the file 1-pack_web_static.py
that distributes archive to webservers
"""
import os.path
...
def do_deploy(archive_path):
    ...
    try:
        #TO upload the archive to the /tmp/ directory of the web server
        put(archive_path, "/tmp/{}".format(file_name))

        #TO Create new directory for release
        run("mkdir -p {}".format(folder_path))

        #To Untar archive
        run("tar -xzf /tmp/{} -C {}".format(file_name, folder_path))

        #To Delete the archive from the web server
        run("rm -rf /tmp/{}".format(file_name))

        #TO Move extraction to proper directory
        run("mv {}web_static/* {}".format(folder_path, folder_path))

        #To Delete first copy of extraction after move
        run("rm -rf {}web_static".format(folder_path))

        #To Delete the symbolic link /data/web_static/current from the web server
